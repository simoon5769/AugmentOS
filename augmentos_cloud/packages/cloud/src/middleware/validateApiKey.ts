...
export const hashWithApiKey = async (req: Request, res: Response, next: NextFunction) => {
  const packageName = req.body.packageName || req.query.packageName || req.params.packageName; // Try to get packageName if available
  ...
  try {
    ...
    if (packageName) {
      ...
      // For now, we'll assume packageName is available somehow.
      // A more robust approach might involve looking up the API key hash directly.
      logger.warn('TPA API Key Middleware: Package name not provided for API key hashing.');
      return res.status(400).json({ error: 'Package name required for API key hashing' });
    }
