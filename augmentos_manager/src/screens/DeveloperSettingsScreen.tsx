...
const DeveloperSettingsScreen: React.FC<DeveloperSettingsScreenProps> = ({
  ...
  const handleSaveUrl = async () => {
    ...
    if (!urlToTest.startsWith('http://') && !urlToTest.startsWith('https://')) {
      showAlert('Invalid URL', 'Please enter a valid URL starting with http:// or https://', [{ text: 'OK' }], { isDarkTheme });
      return;
    }
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        showAlert('Success', 'Custom backend URL saved and verified. It will be used on the next connection attempt or app restart.', [{ text: 'OK' }], { isDarkTheme });
      } else {
        // Handle non-2xx responses as errors
        console.error(`URL Test Failed: Status ${response.status}`);
        showAlert('Verification Failed', `The server responded, but with status ${response.status}. Please check the URL and server status.`, [{ text: 'OK' }], { isDarkTheme });
      }
    } catch (error: unknown) {
      ...
      showAlert('Verification Failed', errorMessage, [{ text: 'OK' }], { isDarkTheme });
    } finally {
